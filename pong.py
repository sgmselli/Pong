...
class Bar():
    ...
    def rightmove(self):
        key_pressed = pygame.key.get_pressed()
        if key_pressed[pygame.K_UP] and Bar.right_y>0:
            Bar.right_y -= self.velocity
        elif key_pressed[pygame.K_DOWN] and Bar.right_y<HEIGHT-70:
            Bar.right_y += self.velocity
...
class Ball():
    ...
    def move(self):
        ...
        if self.y > Bar.right_y and self.y < Bar.right_y+70 and self.x>WIDTH-12:
            self.velocity[0] = -self.velocity[0]
            self.y_movement=True            

        elif self.y > Bar.left_y and self.y < Bar.left_y+70 and self.x<12:
            self.velocity[0] = -self.velocity[0]
            self.y_movement=True
